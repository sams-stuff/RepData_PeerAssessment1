...
The mean/median of the total number of steps taken per day did not change much from when the 2304 NA rows were stripped before computation in part 2-3 to when the NA rows were filled in with the mean values for the intervals from part 4-4.  So I can conclude that for a small dataset, the NAs do not have much of an effect on the mean/median.  Refer to the table below.
