...
Use the *ddply()* function to find the mean total number of steps taken per day.
...
Use the *sum()* and *complete.cases()* functions to determine the number rows with missing values.  The number of rows with missing values is **2304**.
...
The mean/median of the total number of steps taken per day did not change much from when the NAs were stripped before computation in part 2-3 to when the NAs were filled in with the mean values for the intervals from part 4-4.  So I can conclude that for a small dataset, the NAs do not have much of an effect on the mean/median.  Refer to the table below.
...
The daily activity plot comparing weekdays to weekends indicate the subject is more active throughout the day on the weekends.  This might suggest the subject has a desk job.
